...
public class Enemy_path2 extends Entity{
	...
	public Enemy_path2(double x, double y, int width, int height, double speed, int type, BufferedImage sprite) {
	...
	//	}
			...
			for(int i = 0; i < Game.entities.size(); i++) {
				...
				Game.life -= 1;
				Player_path2.playerWasHurt = true;
				return;
			    	}
			}
	        }
	...
	public void render(Graphics g) {
		if(type == 1) {
			...
			if(right) {
			g.drawImage(rightEnemy[index], this.getX(), this.getY(), 16, 16, null);
			}
			else if(left) {
			g.drawImage(leftEnemy[index], this.getX(), this.getY(), 16, 16, null);	
			}
			}else {
		    g.drawImage(frozenEnemy[index], this.getX(), this.getY(), 16, 16, null);	
			}
			}else if(type == 2){
				if(pathSpeed == 100) {
					
					if(right) {
					g.drawImage(rightEnemy2[index], this.getX(), this.getY(), 16, 16, null);
					}
					else if(left) {
					g.drawImage(leftEnemy2[index], this.getX(), this.getY(), 16, 16, null);	
					}
					}else {
				    g.drawImage(frozenEnemy2[index], this.getX(), this.getY(), 16, 16, null);	
					}
				
			}
		
		g.setColor(Color.white);
		g.fillRect((this.getX() - 1), (this.getY() - 5), 18, 5);
		g.setColor(new Color(80, 80, 80, 130));
		g.fillRect((this.getX()), (this.getY() - 4),16,3);
		
		g.setColor(Color.red);
		g.fillRect((this.getX()), (this.getY() - 4),(int)((this.life/this.maxLife)*16),3);
