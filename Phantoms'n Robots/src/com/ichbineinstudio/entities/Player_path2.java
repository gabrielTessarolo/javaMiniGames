...
public class Player_path2 extends Entity{
	...
	public Player_path2(int x, int y, int width, int height,double speed,BufferedImage sprite) {
	...
	public void render(Graphics g) {
		if(width == 32 && height == 32) {
			g.drawImage(tower2[index], this.getX(), this.getY(), 32, 32, null);
			if(isAttacking) {
				g.setColor(new Color(255, 204, 51));
				g.drawLine((int)(x+16), (int)(y+5), xTarget + 8, yTarget);
			}
			}
			else if(width == 16 && height == 16) {
				g.drawImage(littletower2[index], this.getX(), this.getY(), 16, 16, null);
				if(isAttacking) {
					g.setColor(new Color(255, 204, 51));
					g.drawLine((int)(x+8), (int)(y+2), xTarget + 8, yTarget);
				}
			}
	}
